...
if [ $(uname) = "Darwin" ]; then
  clang++ -c gamemaker.cpp apiprocess/process.cpp -shared -std=c++17 -DPROCESS_GUIWINDOW_IMPL -framework Cocoa -framework CoreFoundation -framework CoreGraphics -ObjC++ -arch arm64 -arch x86_64 && clang++ gamemaker.o process.o -o libxprocess.dylib -shared -std=c++17 -DPROCESS_GUIWINDOW_IMPL -framework CoreFoundation -framework CoreGraphics -framework Cocoa -ObjC++ -arch arm64 -arch x86_64;
  ...
  clang++ -c gamemaker.cpp apiprocess/process.cpp -shared -std=c++17 -I/usr/local/include -L/usr/local/lib -lkvm -lc -lpthread -DPROCESS_GUIWINDOW_IMPL `pkg-config x11 --cflags --libs` -fPIC && g++ gamemaker.o process.o -o libxprocess.so -shared -std=c++17 -I/usr/local/include -L/usr/local/lib -lkvm -lc -lpthread -DPROCESS_GUIWINDOW_IMPL `pkg-config x11 --cflags --libs` -fPIC;
  ar rc libxprocess.a gamemaker.o process.o && rm -f "gamemaker.o" "process.o";
else
